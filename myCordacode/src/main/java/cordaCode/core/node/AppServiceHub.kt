package cordaCode.core.node

import cordaCode.core.DeleteForDJVM
import cordaCode.core.flows.FlowLogic
import cordaCode.core.node.ServiceHub
import cordaCode.core.messaging.FlowHandle
import cordaCode.core.messaging.FlowProgressHandle
import cordaCode.core.node.services.ServiceLifecycleEvent
import cordaCode.core.node.services.ServiceLifecycleObserver
import cordaCode.core.node.services.vault.CordaTransactionSupport
import rx.Observable

/**
 * A [cordaCode.core.node.services.CordaService] annotated class requires a constructor taking a
 * single parameter of type [AppServiceHub].
 * With the [AppServiceHub] parameter a [cordaCode.core.node.services.CordaService] is able to access to privileged operations.
 * In particular such a [cordaCode.core.node.services.CordaService] can initiate and track flows marked
 * with [cordaCode.core.flows.StartableByService].
 */
@DeleteForDJVM
interface AppServiceHub : ServiceHub {
     ...
     * result of running the flow. [flow] must be annotated with [cordaCode.core.flows.StartableByService].
     * TODO it is assumed here that the flow object has an appropriate classloader.
     */
    ...
    /**
     * Allows to register [ServiceLifecycleObserver] such that it will start receiving [cordaCode.core.node.services.ServiceLifecycleEvent]s
     *
